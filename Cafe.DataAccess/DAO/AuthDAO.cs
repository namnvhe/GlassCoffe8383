...
namespace Cafe.DataAccess.DAO
{
    public class AuthDAO
    {
        ...
        // Hỗ trợ xóa tài khoản không active sau 7 ngày
        private async Task CleanupInactiveAccountsAsync(string email)
        {
            try
            {
                ...
                var inactiveUser = await _context.Users.SingleOrDefaultAsync(u => u.Email.Equals(email) && u.LastLoginAt == null && u.CreatedAt <= sevenDaysAgo);
        ...
        public async Task<RegisterResponse> RegisterAsync(string fullName, string email, string password, string phone = null, string role = "Customer")
        {
            try
            {
                ...
                // Gửi email chào mừng với thông tin
        ...
        public async Task<ForgotPasswordResponse> ForgotPasswordAsync(string email)
        {
            try
            {
                ...
                if (user != null)
                {
                    ...
                    // Tạo reset link
                    string resetLink = $"https://localhost:7196/ForgotPassword/ResetPassword?token={resetToken}";
        ...
        public async Task<ResetPasswordResponse> ResetPasswordAsync(string token, string newPassword)
        {
            try
            {
                var user = await _context.Users.SingleOrDefaultAsync(u => u.PasswordResetToken.Equals(token) && u.PasswordResetTokenExpiry > DateTime.Now && u.IsActive == true);
        ...
        public async Task<AuthResponse> RefreshAccessTokenAsync(string refreshToken)
        {
            try
            {
                var user = await _context.Users.SingleOrDefaultAsync(u => u.RefreshToken.Equals(refreshToken) && u.RefreshTokenExpiryTime > DateTime.Now && u.IsActive == true);
        ...
        public async Task<bool> VerifyEmailAsync(string token)
        {
            try
            {
                var user = await _context.Users.SingleOrDefaultAsync(u => u.EmailVerificationToken.Equals(token) && u.EmailVerificationTokenExpiry > DateTime.Now);
        ...
        public async Task<User> FindUserByRefreshTokenAsync(string refreshToken)
        {
            try
            {
                return await _context.Users.SingleOrDefaultAsync(u => u.RefreshToken.Equals(refreshToken) && u.RefreshTokenExpiryTime > DateTime.Now && u.IsActive == true);
            }
            catch (Exception e)
            {
                throw new Exception(e.Message);
